...
ms = list(Pi = rep(1/6, 6),
          Mu = array(c(0,  0,SEP,SEP,SEP, SEP,
                       0,  0,SEP,SEP,  0, 0), c(6,2)),
          S  = array(c(50, 0,
                       0, 5,
                       5, 0,
                       0, 50), c(2,2,6)))
...
library(dplyr)
POST = as.data.frame(L[[3]]$post) %>% tbl_df
...
df = df %>%
  mutate(
    Cluster = sprintf('g%d',apply(L[[3]]$post, 1, which.max)))
POST = POST %>%
  mutate(
    Cluster = sprintf('g%d',apply(L[[3]]$post, 1, which.max)))
aPOST = aPOST %>%
...
ggtern.theme = ggtern::theme_bw() + 
  ggtern::theme(
    ...
    legend.position = 'none')
p1 <- ggplot() + 
  stat_contour(data=cm, aes(x=X1, y=X2, z=z), col='black') +
  geom_point(data=df, aes(x=X1, y=X2, col=Cluster)) + 
...
library(grid)
pdf('figures/cluster_post.pdf', width=10, height=10)
grid.newpage() 
