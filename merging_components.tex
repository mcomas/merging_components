...
$\lambda_{\log}$ are not scaled into the interval $\left[0,1\right]$. For scaling these functions we respectively propose  $\phi_{\Delta\text{Entr}}(x) = -x/{\log(\frac{1}{s})}$, $\phi_{\text{dist}}(x) = e^x$ and $\phi_{\log}(x) = \frac{e^x}{1+e^x}$. Despite these functions are not the unique possibilities for the scaling, we selected them due to its reasonable performance in our experiments. The corresponding scaled $S$-value functions are: 
...
Therefore, using the geometric structure of $\mathcal{S}^s$ we can introduce new approaches to decide the number of clusters based on how close are the posterior probability vectors to the vertices and how \emph{well} separated are the posterior probability vectors associated to each cluster.
...
\]
Because $\tau_{i I_a}$ is the highest posterior probability (observation $x_i$ is assigned to $I_a$) $\nu_i$ is positive. We have that the lowest $\nu_i$ is the furthest $\m\tau_{i}$ is from vertex $I_a$. For example, when considering four clusters (top-left) the average of $\nu_i$ for observations assigned to clusters centred  at $(0,0)$ and $(20, 0)$ are $15.83$ and $14.46$ respectively. In contrast, the average of $\nu_i$ for observations assigned to clusters centred at $(10, 10\sqrt{3})$ are significatively lower ($2.15$ and $4.00$). When three clusters are considered (bottom-left), the average of $\nu_i$ for observations assigned to clusters centred at at $(0,0)$, $(20, 0)$ and $(10, 10\sqrt{3})$ are 15.82, 14.46 and 19.48 respectively.


To analyse how well separated the posterior probability vectors are we propose to calculate the Calinski-Harabasz (G1) and the Goodman and Kruskal (G2) indices \citep{milligan1985} using the Aitchison distance in $\mathcal{S}^s$. For example, the indices $G1$ and $G2$ respectively take the values $350.28$ and $0.68$ for four clusters in the Figure~\ref{cluster_post} (top-right), whereas for three clusters the corresponding values are $759.49$ and $0.85$ (bottom-right). This differences indicate that the three clusters seems to be a better fit for the sample.

 Therefore, to decide how close are the clusters to the vertex of the Simplex we propose the following: for each pair of cluster $I_a$ and $I_b$ we calculate the average of the log-ratio between $\tau_{\cdot I_a}$ and $\tau_{\cdot I_b}$ for those observations classified to cluster $I_a$.
 
  Let $\mathcal{V}$ be the minimum of the calculated averages. In terms of the geometry of the Simplex, $\mathcal{V}$ can be interpreted as the average distance of the posterior probabilities classified to one cluster to the region where those observations can be classified to another cluster. Formally, $\mathcal{V}$ can be written as\footnote{aquí fem mitjanes de ratios entre vectors columna però el que volem que estigui a prop de vèrtex són vectors fila...no caldria fer-ho per files? Es a dir, mirar per cada fila la mínima ratio i després fer la mitjana...
