...
\Affiliations{
\item Department of Computer Sciences, Mathematics and Statistics, 
      Polytechnic School, 
      University of Girona,
      Spain
}

\CorrAddress{Marc Comas Cufí, Department of Computer Science, Applied Mathematics and Statistics,
      University of Girona, P4-Campus Montilivi, 17071 Girona, Spain}
\CorrEmail{mcomas@imae.udg.edu}
\CorrPhone{(+34)\;972\; 418\;217}
\CorrFax{(+34)\;972\;418\;792}

\Abstract{
A common approach in parametric cluster analysis assumes data can be modelled by means of a finite mixture of distributions. Different authors have shown that associating each mixture component to one cluster can be misleading because frequently two different mixture components are so overlapped that they can be considered as a unique cluster. In other words, one cluster could be formed by the result of merging two different mixture components. In this article we present a generic approach to build a hierarchy of components in such a way that those components that are more reliable to form a single cluster a merged earlier. The presented approach generalize methods presented by other authors. Using the generic approach we introduce a new approach based in the log-ratio of posterior probabilities.
}
