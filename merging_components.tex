...
\Abstract{
Methods in parametric cluster analysis commonly assume data can be modelled by means of a finite mixture of distributions. However associating each mixture component to one cluster is frequently misleading because different mixture components can overlap forming a unique cluster. A number of approaches have already been proposed to construct the clusters by merging components using the posterior probabilities. In this paper a generic approach to build a hierarchy of mixture components that integrates and generalises some techniques earlier proposed in the literature is presented.  Using this proposal a new technique based on the log-ratio of posterior probabilities is proposed. Moreover, to decide the final number of clusters two new methods are introduced. Simulated and real datasets are used to illustrate this methodology.
}
...
For a sample formed by $s$ well separated clusters, when each cluster is modelled by a different probability distribution, the posterior probability vectors of the elements of each cluster define a cluster close to a vertex in $\mathcal{S}^s$. That is, one value in these posterior probability vectors is approximately equal to one whereas the rest of values are close to zero. For example, in Figure~\ref{cluster_post} we have a sample generated following a mixture of four gaussian distributions. In Figure~\ref{cluster_post} (top-left) we cluster the sample in four clusters where each cluster is modelled by a single gaussian distribution. If we represent the posterior probability vectors on a quaternary diagram (top-right), we see that the posterior probabilty vectors for clusters associated to first and third components are located at the vertex of $\mathcal{S}^4$ and well separated from the rest. In contrast, we see that the posterior probability vectors for clusters associated to second and fourth components are not well separed between them. The situation changes if we separate the sample in three clusters (bottom-left), where one cluster (squared symbols) is modelled by the mixture of two gaussian distributions. If we represent the posterior probability vectors in a ternary diagram (bottom-right), we see that posterior probabilities vectors form respectively three separate clusters located at the corresponding vertices of $\mathcal{S}^3$. 
...
Using the geometric structure of $\mathcal{S}^s$  we can introduce another approach to decide the number of \emph{clusters of observations} based on \textcolor{red}{how \emph{well} separated are the \emph{clusters of posterior probability vectors}. To this purpose, we study how \emph{well} components $f_{I_1}, \dots, f_{I_s}$ define clusters of observations using distance based heuristics on the clusters of posterior probability vectors. To analyse the clusters formed by the posterior probability vectors we calculate the Calinski-Harabasz (G1) and the Goodman and Kruskal (G2) indices \citep{milligan1985}. For example, the indices $G1$ and $G2$ respectively take the values $350.28$ and $0.68$ for four clusters in the Figure~\ref{cluster_post} (top-right), whereas for three clusters the corresponding values are $759.49$ and $0.85$ (bottom-right).} This difference indicates that three clusters seems a better fit the data. We need to confirm that the defined clusters are located on the vertices of $\mathcal{S}^s$.\footnote{Què sortiria si G1 i G2 s'aplica a les dades? Aleshores caldria definir la distància adeqüada a les observacions. Mirant G1 i G2 sobre els posterioris es considera la distància d'Aitchison sobre la distribució triada.}

Let $I_1,\dots, I_s$ be the parts defining the $s$ clusters. The posterior probability vectors of observations $x_i$ assigned to $I_a$ are close to the corresponding vertex if $\tau_{i I_a}$ is close to one. In addition, $\tau_{i I_b}$, $b \neq a$, are close to zero. In consequence, the log-ratio $\log(\tau_{i I_a}/\tau_{iI_b}$) should take larger values for all observations assigned to cluster $I_a$. Therefore, to decide how close are the clusters to the vertex of the Simplex we propose the following: for each pair of cluster $I_a$ and $I_b$ we calculate the average of the log-ratio between $\tau_{\cdot I_a}$ and $\tau_{\cdot I_b}$ for those observations classified to cluster $I_a$. Let $\mathcal{V}$ be the minimum of the calculated averages. In terms of the geometry of the Simplex, $\mathcal{V}$ can be interpreted as the average distance of the posterior probabilities classified to one cluster to the region where those observations can be classified to another cluster. Formally, $\mathcal{V}$ can be written as\footnote{aquí fem mitjanes de ratios entre vectors columna però el que volem que estigui a prop de vèrtex són vectors fila...no caldria fer-ho per files? Es a dir, mirar per cada fila la mínima ratio i després fer la mitjana...
no sé si dóna igual el mínim de les mitjanes que les mitjanes dels mínims! ¿?¿??¿¿}
\[
...
\]
A different interpretation of $\mathcal{V}$ is by means of relation $\mathcal{V} = - \mathcal{S}_{\omega_\text{dich}, \lambda_{\log}}$. Suggesting that criteria $\mathcal{V}$ is measuring the higher confusion between two clusters. In this scenario, the higher $\mathcal{V}$  is  the closer the clusters are to the vertex. For example, the score is $1.64$ for four clusters in the Figure~\ref{cluster_post} (top-right) and $14.66$ for three clusters (bottom-right). Suggesting that after merging from four to three components the three clusters are close all close to one vertex of $\mathcal{S}^3$. We call this criteria \emph{the closeness to the vertex criteria}.

{\color{red}When some cluster of posterior prob vectors is not close to its corresponding vertex we expect that $\mathcal{V}$ takes a value close to zero. Once, for the first time, all these clusters are close to its vertex the value of $\mathcal{V}$ will take a large value. After this step, $\mathcal{V}$ will incresase its value. To detect the number of cluster resumes to detect the first time that $\mathcal{V}$ takes a large positive value.}
...
\begin{tabular}{cc}
 ...
 \end{tabular}
\caption{Different criterias are shown to decide the number of clusters for the sample generated with a finite mixture of six Gaussian components. At the top of the figure the $S$-values of different approaches are shown. In order by rows: the DEMP approach \citep{hennig2010methods}, the modified DEMP \citep{longford2014}, the difference of entropies \citep{baudry2010combining}, the log-ratio approach with proportional weighing (i,e., $\mathcal{S}_{\omega_\text{prop}, \lambda_{\log}}$),  the Aitchison distance with proportional weighing (i,e., $\mathcal{S}_{\omega_\text{prop}, \lambda_{\text{dist}}}$), the difference of enrtropies approach scaled and the log-ratio approach with proportional weighing scaled. At the bottom part the distance based criterias for the posterior probability vectors are shown. In order: the Calinski-Harabasz and Goodman-Kruskal criteria, and the closeness to the vertex criteria.}\label{gaussian_Svalues}
\end{center}
\end{figure}
...
\begin{tabular}{cc}
 ...
 \end{tabular}
\caption{Different criterias are shown to decide the number of clusters for the pigs sample. At the top of the figure the $S$-values of different approaches are shown. In order by rows: the DEMP approach \citep{hennig2010methods}, the modified DEMP \citep{longford2014}, the difference of entropies \citep{baudry2010combining}, the log-ratio approach with proportional weighing (i,e., $\mathcal{S}_{\omega_\text{prop}, \lambda_{\log}}$),  the Aitchison distance with proportional weighing (i,e., $\mathcal{S}_{\omega_\text{prop}, \lambda_{\text{dist}}}$), the difference of enrtropies approach scaled and the log-ratio approach with proportional weighing scaled. At the bottom part the distance based criterias for the posterior probability vectors are shown. In order: the Calinski-Harabasz and Goodman-Kruskal criteria, and the closeness to the vertex criteria.}\label{multinomial_statistics}
\end{center}
\end{figure}
